...
public class DailyTasks {
    ...
    * <code>java -jar target/04-queue-1.0-SNAPSHOT-jar-with-dependencies.jar</code>
    * 
    * @param args Not used.
    */
   ...
   private void run() {
      try {
         // TODO:
         // 1. create a queue (to the member variable!) for daily tasks, which are
         // strings.
         dailyTaskQueue = QueueFactory.createStringQueue();
         // 2. read the tasks for today by calling readTasks() -- implementing missing
         // parts of it!
         readTasks();
         // 3. create Java Timer object (to member variable) to schedule your daily
         // tasks. (Already given to you.)
         timer = new Timer();
         // 4. schedule the timer at fixed rate with a new TimerTask,
         // using the delay constant values in the class member variable. (Already given
         // to you.)
   ...
   private void readTasks() throws IOException {
      ...
      System.out.println("The task is" + dailyTaskQueue.size());
   }
